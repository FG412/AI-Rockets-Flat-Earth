...
public class HopAgent : Agent
{
    ...
    public override void OnEpisodeBegin()
    {
        ...
        target.transform.localPosition = new Vector3(Random.Range(-5, 5), 19 + Random.Range(0, 10), Random.Range(-5, 5));
    }
    ...
    void FixedUpdate(){
        Debug.Log(StepCount);
        if (StepCount == 1000){
            target.transform.position = new Vector3(rocket.transform.position.x, -1.805f, rocket.transform.position.z);
            rocket.setTriggerLegsDeploy(true);
            GetComponent<DecisionRequester>().DecisionPeriod = 1;
            SetModel("LandingPilotAgent", landingModel);
        }
    }
